...
export function Sidebar({ open }: SidebarProps) {
  ...
  return (
    <aside
      className={`fixed left-0 top-14 bottom-0 w-72 bg-card text-foreground z-40 flex flex-col shadow-xl transition-transform duration-300 ease-out origin-left border-r border-border ${
    ...
    >
        <div className="p-2 flex gap-2 shrink-0">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-muted hover:bg-accent text-sm font-medium transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-2 min-h-0">
          <div className="mb-4">
            <button
              ...
              className="w-full flex items-center gap-2 text-sm font-medium text-foreground px-2 py-2 rounded-lg hover:bg-accent/50 transition-colors"
            >
              {chatExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                  ...
                  >
                    {editingId === s.id ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          className="p-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity hover:bg-accent text-muted-foreground shrink-0"
                          title="제목 변경"
                          aria-label="제목 변경"
                        >
                          <Pencil size={14} />
                    ...
                    {editingId !== s.id && (
                      <button
                        ...
                        className="p-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive/20 text-muted-foreground hover:text-destructive shrink-0"
                        title="채팅 삭제"
                        aria-label="채팅 삭제"
                      >
                        <Trash2 size={14} />
          ...
          <div>
            <button
              ...
              className="w-full flex items-center gap-2 text-sm font-medium text-foreground px-2 py-2 rounded-lg hover:bg-accent/50 transition-colors"
            >
              {imageExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
